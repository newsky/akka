...
class ActorSelectionSpec extends AkkaSpec("akka.loglevel=DEBUG") with DefaultTimeout {
  ...
  "An ActorSystem" must {
    ...
    /* FIXME #3074 do we need to support temp actors? That would require the SelectChildName dance in VirtualPathContainer also
    "find temporary actors" in {
      val f = c1 ? GetSender(testActor)
      val a = expectMsgType[ActorRef]
      a.path.elements.head must be === "temp"
      identify(a.path) must be === Some(a)
      identify(a.path.elements.mkString("/", "/", "")) must be === Some(a)
      identify(a.path.elements.mkString("/", "/", "") + "/") must be === Some(a)
      identify(a.path.elements.mkString("/", "/", "") + "/hallo") must be === None
      ...
      awaitCond(identify(a.path) == None, 1 second)
    }
    */
  ...
  "An ActorContext" must {
    ...
    "find temporary actors" in {
      ...
      val a = expectMsgType[ActorRef]
      a.path.elements.head must be === "temp"
      askNode(c2, SelectPath(a.path)) must be === Some(a)
      askNode(c2, SelectString(a.path.elements.mkString("/", "/", ""))) must be === Some(a)
      askNode(c2, SelectString(a.path.elements.mkString("/", "/", ""))) must be === Some(a)
      askNode(c2, SelectString("../../" + a.path.elements.mkString("/"))) must be === Some(a)
      askNode(c2, SelectString(a.path.elements.mkString("/", "/", "") + "/")) must be === Some(a)
      askNode(c2, SelectString(a.path.elements.mkString("/", "/", "") + "/")) must be === Some(a)
      ...
      Await.result(f, timeout.duration) must be === 42
      // clean-up is run as onComplete callback, i.e. dispatched on another thread
      awaitCond(askNode(c2, SelectPath(a.path)) == None, 1 second)
    }
    */
